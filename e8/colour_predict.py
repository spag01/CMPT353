...
def main():
    ...
    # For bayes_rgb_model predicted with Guassian classiffier
    bayes_rgb_model = GaussianNB()
    bayes_rgb_model.fit(X_train, y_train)

    # For knn_rgb_model predicted with Nearest neighbor classifier
    knn_rgb_model = KNeighborsClassifier(n_neighbors=12)
    knn_rgb_model.fit(X_train, y_train)

    # For bayes_convert_model predicted with Guassian classiffier
    bayes_convert_model = make_pipeline(FunctionTransformer(rgb2lab),GaussianNB())
    bayes_convert_model.fit(X_train, y_train)

    # For knn_convert_model predicted with Nearest neighbor classifier
    knn_convert_model = make_pipeline(FunctionTransformer(rgb2lab),KNeighborsClassifier(n_neighbors=12))
    knn_convert_model.fit(X_train, y_train)

    # For rf_rgb_model predicted with Random Forest classifer
    rf_rgb_model = RandomForestClassifier(n_estimators=450, min_samples_leaf=6)
    rf_rgb_model.fit(X_train, y_train)
    
    # For rf_conver_model predicted with Random Forest classifer
    rf_convert_model = make_pipeline(FunctionTransformer(rgb2lab),RandomForestClassifier(n_estimators=450, min_samples_leaf=6))
